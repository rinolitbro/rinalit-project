	...
	private void Update()
	{
		if (Input.GetMouseButtonDown(0) && Time.timeScale != 0f)
		{
			...
			if (Physics.Raycast(ray, out raycastHit))
			{
				...
				else if (raycastHit.transform.name == "Pickup_Soda" & Vector3.Distance(this.player.position, base.transform.position) < 10f)
				{
					raycastHit.transform.gameObject.SetActive(false);
					this.gc.CollectItem(11);
				}
				else if (raycastHit.transform.name == "Pickup_Circles" & Vector3.Distance(this.player.position, base.transform.position) < 10f)
				{
					raycastHit.transform.gameObject.SetActive(false);
					gc.score += 5;
				}

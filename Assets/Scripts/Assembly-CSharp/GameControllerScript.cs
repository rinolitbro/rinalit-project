...
public class GameControllerScript : MonoBehaviour
{
	...
	private void Update()
	{
		...
		{
			if (Input.GetButtonDown("Pause") && !backpackOpen && !bombMenuActive()) 
			{
				if (!this.gamePaused)
				{
					this.PauseGame();
			...
			if (Input.GetKeyDown(KeyCode.Y) & this.gamePaused & ! backpackOpen)
			{
				this.ExitGame();
			}
			else if (Input.GetKeyDown(KeyCode.N) & this.gamePaused & !backpackOpen)
			{
				this.UnpauseGame();
			}
			else if (Input.GetKeyDown(KeyCode.R) & this.gamePaused & !backpackOpen)
			{
				this.RestartGame();
			}
		...
		if (this.player.stamina == -5f & !this.warning.activeSelf)
		{
		...
		if (Input.GetKeyDown(KeyCode.Z) && !this.gamePaused)
		{
			if (!backpackOpen)
			{
				this.PauseBackpack();
			...
			{
				this.UnpauseBackpack();
			}
		}
		else if (Input.GetKeyDown(KeyCode.Z) & this.gamePaused)
		{
			this.UnpauseBackpack();
		}
		if (this.notebooks == 10 & this.mode == "story" & !AudioPlay10items) // Plays the all 7 notebook sound
		{
			audioDevice.PlayOneShot(aud_AllNotebooks, 0.8f);
			AudioPlay10items = true;
			phone.SetActive(true);
			buttonBomb1.interactable = true;
		}
		if (GameObject.Find ("item"))
        {
			player.ResetGuilt("bomb", 0.1f);
        }
		scoreText.text = score.ToString() + " Circles";
	}
	...
	private void UpdateNotebookCount()
	{
		if (this.mode == "story")
		{
			this.notebookCount.text = this.notebooks.ToString() + "/10 Items";
		}
		else
		{
			this.notebookCount.text = this.notebooks.ToString() + " Items";
		}
		if (this.notebooks == 10 & this.mode == "story")
		{
	...
	public void PauseBackpack()
	{
		if (!this.learningActive && !gamePaused && !inShop)
		{
			Time.timeScale = 0;
			this.gamePaused = true;
			backpackOpen = true;
			this.UnlockMouse();
			this.pause2Menu.SetActive(true);
		}
	}
	public void UnpauseBackpack()
	{
		if (!this.learningActive && pause2 == 0 && !inShop)
		{
			gamePaused = false;
			Time.timeScale = 1;
			backpackOpen = false;
			this.LockMouse();
			this.pause2Menu.SetActive(false);
		}
	}
	...
	public void ActivateSpoopMode()
	{
	    ...
	    this.Finya.SetActive(true); //Turns on Test-Enemy
		FlySmoke.SetActive(true);
	...
	public void ShopPause()
	{
		if (!learningActive && !gamePaused)
		{
			...
			finyaother.enabled = false;
		}
	}
	public void UnpauseShop()
	{
		if (!this.learningActive && pause2 == 0)
		{
			gamePaused = false;
			Time.timeScale = 1;
			this.LockMouse();
			...
			finyaother.enabled = true;
		}
	}
	bool bombMenuActive()
	{
		if (GameObject.Find("BombMenu")) return true;
		else return false;
	}
	...
	// Token: 0x0400060D RID: 1549
	public GameObject Finya;

	public GameObject FlySmoke;
	...
	public Button buttonBomb1;

	public TMP_Text scoreText;

	public int score;
	public AudioSource finyaother;
}
